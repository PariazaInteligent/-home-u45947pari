...
$attachReply = function (array $row) use (&$replyCache, $pdo, $has, $textCol, $roomVal): array {
  if (!$has['reply_to'] || !isset($row['reply_to']))
    return $row;
  ...
  try {
    ...
    if ($has['room'])
      $st->bindValue(':room', $roomVal, PDO::PARAM_STR);
    $st->execute();
    $snap = $st->fetch();
    if ($snap) {
      $replyCache[$rid] = [
...
// ——— helper: atașează meta din DB (atașamente + preview link) ———
$attachMeta = function (array $row) use ($pdo): array {
  $mid = (int) ($row['id'] ?? 0);
  if ($mid <= 0)
    return $row;
  ...
  // Attachments
  try {
    $st = $pdo->prepare("SELECT url, name, mime, kind, size FROM chat_attachments WHERE message_id = ?");
    $st->execute([$mid]);
  ...
  // Link Preview
  try {
    $st = $pdo->prepare("SELECT data_json FROM chat_link_previews WHERE message_id = ?");
    $st->execute([$mid]);
    $json = $st->fetchColumn();
    if ($json) {
      $dec = json_decode($json, true);
      if ($dec)
        $row['link_preview'] = $dec;
    }
  } catch (Throwable $e) {
  }
