...
<body class="min-h-screen bg-slate-950 text-slate-100"
  ...
  async function fetchApiBets(){
    ...
    try{
      ...
      return makeDemoBets(250);
    }
  }

  // ===== Demo generator fallback
  function makeDemoBets(n){
    const sports=['Fotbal','Baschet','Tenis','Hochei','Esports'];
    const books=['StripeBet','Comet','Orbital','Nova'];
    const leagues={
      Fotbal:['UCL','Liga 1','Serie A','PL'],
      Baschet:['EuroLeague','NBA'],
      Tenis:['ATP','WTA'],
      Hochei:['NHL','KHL'],
      Esports:['CS2','LoL']
    };
    const statuses=['WON','LOST','PENDING','VOID','HALF_WON','HALF_LOST'];
    const out=[];
    const now=Date.now();
    for(let i=0;i<n;i++){
      const sport=sports[Math.floor(Math.random()*sports.length)];
      const league=leagues[sport][Math.floor(Math.random()*leagues[sport].length)];
      const decOdds= +(1.3+Math.random()*2.8).toFixed(2);
      const stake = +(5+Math.random()*95).toFixed(2);
      const ts = now - Math.floor(Math.random()*1000*60*60*24*180);
      const book=books[Math.floor(Math.random()*books.length)];
      const status = statuses[Math.floor(Math.random()*statuses.length)];

      let ret=0;
      if(status==='WON')        ret= +(stake*decOdds).toFixed(2);
      else if(status==='VOID')  ret=stake;
      else if(status==='HALF_WON')  ret= +(stake + (stake*(decOdds-1))/2).toFixed(2);
      else if(status==='HALF_LOST') ret = +(stake/2).toFixed(2);
