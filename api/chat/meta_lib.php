...
function chat_normalize_attachment(array $in): ?array {
  ...
  if (!str_starts_with($url, '/uploads/chat/')) return null;
  ...
  foreach ($allowed as $k=>$list) {
    if (in_array($ext, $list, true)) { $kind = $k; break; }
  }
  if ($mime === '' && $kind !== 'file') $mime = $kind . '/' . $ext;
  return [
    'url'  => $url,
...
function chat_filter_attachments($list): array {
  ...
  foreach ((array)$list as $it) {
    ...
    if ($norm) $out[] = $norm;
    if (count($out) >= 5) break; // limită de siguranță
  }
  return $out;
}
