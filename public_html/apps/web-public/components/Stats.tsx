...
export const Stats: React.FC = () => {
   ...
   const transactions = metrics?.signals || [];

   const stats = metrics ? [
      {
         label: "Bancă Totală",
         value: `${metrics.equity} EUR`,
         color: "text-white",
         sub: metrics.equityGrowth || "N/A"
      },
      {
         label: "Profit Istoric",
         value: metrics.totalProfit,
         color: metrics.totalProfit?.includes('-') ? "text-red-400" : "text-emerald-400",
         sub: "Calculat după taxe"
      },
      {
         label: "Investitori",
         value: metrics.investorCount?.toString() || "N/A",
   ...
   return (
      <section id="stats" className="py-32 bg-slate-950 relative border-t border-white/5">
         ...
         <div className="max-w-7xl mx-auto px-6 relative z-10">
            {/* API Error Display */}
            {error && (
               <div className="mb-8 bg-red-900/20 border-2 border-red-500/50 rounded-xl p-6 backdrop-blur-sm">
                  <div className="flex items-center gap-4">
                     <div className="text-red-500 text-4xl">⚠️</div>
                     <div>
                        <div className="text-red-400 font-bold text-xl mb-1">Eroare de Conexiune</div>
                        <div className="text-red-300 text-sm">Nu se pot încărca datele de la server. Verifică că API-ul rulează pe portul 3001.</div>
                        <div className="text-red-400/60 text-xs mt-2 font-mono">{error}</div>
                     </div>
                  </div>
               </div>
            )}
