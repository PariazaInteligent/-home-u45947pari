...
export const AdminConsolePage: React.FC<AdminConsolePageProps> = ({ onLogout, onSwitchToInvestor, onNavigateToProfile }) => {
  ...
  const activeTab = (() => {
    ...
    if (!lastPart || lastPart === 'admin') return 'overview';
    return lastPart;
  })();
              ...
              <div className="space-y-1">
                {group.items.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => {
                      // Fix: System Status (overview) should point to root /admin
                      const path = item.id === 'overview' ? '/admin' : `/admin/${item.id}`;
                      navigate(path);
                      setIsMobileMenuOpen(false);
                    }}
                    className={`w-full flex items-center p-2.5 rounded-lg transition-all duration-200 group relative overflow-hidden ${activeTab === item.id
