    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          ...
          throw new Error('Contul nu este activ. Contactează administratorul pentru aprobare.');
      ...
      // Success
      const role = data.user.role === 'ADMIN' || data.user.role === 'SUPER_ADMIN' ? 'admin' : 'investor';
