...
import { UserCheck, Shield, ChevronRight, CheckCircle2, Terminal, ArrowLeft, Lock, Fingerprint, Key, Unlock, Clock, AlertTriangle } from 'lucide-react';
...
export const RegisterPage: React.FC<RegisterPageProps> = ({ onBack, onSwitchToLogin, onLoginSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanCode = formData.inviteCode.trim().toUpperCase();
    
    if (cleanCode === 'PRO-2025') {
              ...
              <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <Key className="absolute left-4 top-3.5 w-5 h-5 text-slate-500 group-focus-within:text-yellow-400 transition-colors" />
                      <input 
                         type="text" 
                         name="inviteCode"
                    ...
                    <div className="flex items-center gap-2 mt-2 bg-slate-900/50 p-2 rounded border border-white/5">
                       <span className="text-[10px] text-cyan-500 font-mono uppercase tracking-wider">Developer Hint:</span>
                       <span className="text-xs text-slate-300 font-mono select-all cursor-pointer hover:text-white" title="Copy code">PRO-2025</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    <Button3D variant="cyan" className="w-full">
                        <span className="flex items-center gap-2">
                          INIȚIAZĂ CONEXIUNEA <ChevronRight className="w-4 h-4" />
                        </span>
                    </Button3D>
                  </div>
