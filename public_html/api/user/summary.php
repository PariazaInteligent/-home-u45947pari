...
if ($available_alltime_cents < 0) {
  $available_alltime_cents = 0;
}
...
echo json_encode([
  ...
  // --- chei NOI pentru Lumen Insight / Growth Modal ---
  // Avem nevoie de retragerile din fereastra selectată pentru a calcula "Profit rămas"
  'withdrawals_cents' => sum_withdraw_total_out($pdo, $userId, 'APPROVED'), // momentan e ALL TIME, dar pentru Growth Modal e ok dacă folosim logica de "Retrageri & taxe (estimate)" care e de obicei all-time în contextul "sold curent". 
  // DAR, userul vrea "Retrageri & taxe (estimate)" în modal. Dacă selectează "Today", ar trebui să fie retragerile de azi?
  // Userul zice: "Randament (strategie) arată performanța fără a penaliza retragerile aprobate."
  // Și "Profit rămas (după retrageri)" = Profit Brut - Retrageri.
  // Dacă selectez "Today", Profit Brut e profitul de azi. Retragerile ar trebui să fie cele de azi?
  // Dacă da, trebuie să filtrez sum_withdraw_total_out pe dată.
  // Funcția sum_withdraw_total_out nu are parametri de dată. O voi modifica sau voi face un query ad-hoc aici.
  // Pentru simplitate și robustețe, voi adăuga un query ad-hoc pentru withdrawals in window.

  'withdrawals_window_cents' => (function ($pdo, $uid, $from, $to) {
    $where = "user_id = :uid AND status='APPROVED'";
    if ($from)
      $where .= " AND created_at >= :from";
    if ($to)
      $where .= " AND created_at < :to";
    $sql = "SELECT COALESCE(SUM(amount_cents + fee_cents),0) FROM withdrawal_requests WHERE $where";
    $st = $pdo->prepare($sql);
