...
try {
  ...
  // --- generare notificari pentru mention @username ---
  // extrage toate mentiunile din text (@username)
  $mentionsIds = [];
  $mentionNames = [];

  if ($text !== '') {
    // regex pentru @username (litere, cifre, underscore, punct)
    if (preg_match_all('/@([a-zA-Z0-9_.]+)/', $text, $matches)) {
      $rawMentions = array_unique($matches[1]); // elimina duplicatele

      if (!empty($rawMentions)) {
        // cauta utilizatorii in baza de date dupa name sau email prefix
        $placeholders = implode(',', array_fill(0, count($rawMentions), '?'));
        $sqlMentions = "SELECT id, 
                               COALESCE(NULLIF(TRIM(name),''), SUBSTRING_INDEX(email,'@',1)) AS display_name
                        FROM users 
                        WHERE LOWER(COALESCE(NULLIF(TRIM(name),''), SUBSTRING_INDEX(email,'@',1))) IN (" .
          implode(',', array_fill(0, count($rawMentions), 'LOWER(?)')) . ")";

        $stMentions = $pdo->prepare($sqlMentions);
        $stMentions->execute(array_values($rawMentions));
        $foundUsers = $stMentions->fetchAll();

        foreach ($foundUsers as $user) {
          $userId = (int) $user['id'];
          $displayName = trim((string) $user['display_name']);

          // nu adauga duplicat si nu adauga autorul mesajului
          if ($userId > 0 && !in_array($userId, $mentionsIds, true)) {
            $mentionsIds[] = $userId;
            if ($displayName !== '') {
              $mentionNames[] = $displayName;
            }
          }
        }
      }
  ...
  try {
    ...
    foreach (['message', 'body', 'created_at'] as $c)
      $has[$c] = true; // fallback
  }
  ...
  if (!$textCol) {
    echo json_encode(['ok' => false, 'error' => 'server_error', 'hint' => 'no_text_column']);
    exit;
  }
  ...
  if ($has['client_id']) {
    $cols[] = 'client_id';
    $vals[] = ':cid';
    $bind[':cid'] = ($cid !== '') ? $cid : null;
  }
  if ($has['reply_to']) {
    $cols[] = 'reply_to';
    $vals[] = ':replyto';
    $bind[':replyto'] = $replyTo ?: null;
  }
  if ($has['created_at']) {
    $cols[] = 'created_at';
    $vals[] = 'FROM_UNIXTIME(:ts)';
    $bind[':ts'] = $now;
  }
  ...
  if ($has['mentions_json']) {
    $cols[] = 'mentions_json';
    $vals[] = ':mjson';
    $bind[':mjson'] = $mentionsJSON;
  } else {
    if ($has['mentions_ids']) {
      $cols[] = 'mentions_ids';
      $vals[] = ':mids';
      $bind[':mids'] = json_encode($mentionsIds);
    }
    if ($has['mentions_names']) {
      $cols[] = 'mentions_names';
      $vals[] = ':mnames';
      $bind[':mnames'] = json_encode($mentionNames, JSON_UNESCAPED_UNICODE);
    }
  }
  ...
  if ($id > 0) {
    // 1️⃣ adăugăm mențiunile @username (dacă avem)
    if (!empty($mentionsIds)) {
      foreach ($mentionsIds as $mid) {
        $mid = (int) $mid;
        if ($mid > 0 && $mid !== $uid) {
          $notifRows[] = [
    ...
    // 2️⃣ notificare pentru reply (dacă există)
    ...
    // 3️⃣ salvăm notificările (fie mențiuni, fie reply)
    if (!empty($notifRows)) {
      insertChatNotifications($pdo, $notifRows);
    }
  }

  // Salvăm atașamentele în DB
  if ($attachments) {
    // Schema check (lazy creation)
    ...
    $stAtt = $pdo->prepare("INSERT INTO chat_attachments (message_id, url, name, mime, kind, size) VALUES (?, ?, ?, ?, ?, ?)");
  ...
  // Salvăm link preview în DB
  if ($preview) {
    // Schema check
    $pdo->exec("CREATE TABLE IF NOT EXISTS chat_link_previews (
    ...
    $stPrev = $pdo->prepare("INSERT INTO chat_link_previews (message_id, url, title, description, image, site_name, data_json) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stPrev->execute([
      $id,
      $preview['url'] ?? '',
      $preview['title'] ?? '',
  ...
  // chat_save_meta fallback (optional: we can keep it for backup or remove it. Instructions implied moving to DB).
  // We will REMOVE file storage to follow the instruction "lega-le acum de DB".
  // However, we still need to constructing the response... which we do below.
  ...
  if ($mentionsIds && count($mentionsIds) === count($respMentions)) {
    foreach ($mentionsIds as $k => $mid) {
      if (isset($respMentions[$k]))
        $respMentions[$k]['user_id'] = (int) $mid;
    }
  }
