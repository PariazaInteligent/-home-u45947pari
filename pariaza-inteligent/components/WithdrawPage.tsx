...
export const WithdrawPage: React.FC<WithdrawPageProps> = ({ onBack }) => {
   ...
   const handlePercentage = (percent: number) => {
      ...
      SoundManager.play('click'); // 🎵 Percentage button click
   };

   const handleWithdraw = () => {
      if (!amount) return;
      if (parseFloat(amount) > availableBalance) {
         SoundManager.play('error'); // 🎵 Insufficient balance error
         return;
      }
      ...
      setTimeout(() => {
         setProcessing(false);
         setSuccess(true);
         SoundManager.play('success'); // 🎵 Withdrawal request successful
      }, 2500);
   };
