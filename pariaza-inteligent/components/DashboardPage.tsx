  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'community':
        return <Community />;
      case 'support':
        return <Support />; // Use real component
      case 'settings':
        return <Settings />;
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-white/5">
          <div className="flex items-center">
            ...
            <span className="ml-3 font-display font-bold text-white tracking-widest text-sm">
              PARIAZĂ<span className="text-cyan-400">INTELIGENT</span>
            </span>
          </div>
        ...
        <nav className="flex-1 py-6 space-y-1 px-4 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center p-3 rounded-xl transition-all duration-200 group relative overflow-hidden mb-1 ${
                activeTab === item.id 
                        ...
                        <div key={notif.id} className={`p-3 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer ${!notif.read ? 'bg-cyan-500/5' : ''}`}>
                          <div className="flex gap-3">
                            <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${
                              notif.type === 'profit' ? 'bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]' : 
