...
export const Settings: React.FC = () => {
   ...
   useEffect(() => {
      ...
      if (token) {
         fetch('http://localhost:3001/api/users/preferences', {
            ...
            .then(data => {
               if (data.success && data.preferences) {
                  setSettingsForm(prev => ({
                     ...prev,
                     emailNotifs: data.preferences.emailNotifications ?? prev.emailNotifs,
                     uiSounds: data.preferences.uiSounds ?? prev.uiSounds
                  }));
               }
            })
   ...
   const toggleSetting = async (key: keyof typeof settingsForm) => {
      const newValue = !settingsForm[key];
      setSettingsForm(prev => ({ ...prev, [key]: newValue }));
         ...
         if (token) {
            try {
               const res = await fetch('http://localhost:3001/api/users/preferences', {
                  ...
                  body: JSON.stringify({ uiSounds: newValue })
               });
               ...
               if (res.ok) {
                  ...
                  console.log('[Settings] ✅ uiSounds updated:', newValue);
               } else {
                  console.error('[Settings] ❌ Failed to update ui Sounds');
                  setSettingsForm(prev => ({ ...prev, [key]: !newValue }));
               }
            } catch (err) {
               console.error('[Settings] ❌ Error updating uiSounds:', err);
               setSettingsForm(prev => ({ ...prev, [key]: !newValue }));
            }
         }
      }
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-3 flex flex-col gap-6">
               ...
               {/* Profile Card - Restored Height & Grandeur */}
               <TiltCard glowColor="purple" noPadding className="relative overflow-hidden flex-shrink-0">
                  <div className="absolute top-0 w-full h-24 bg-gradient-to-b from-purple-900/40 to-transparent"></div>

                  <div className="p-6 relative z-10 flex flex-col items-center">
                     {/* Avatar */}
                     <div className="w-24 h-24 rounded-full bg-slate-900 border-2 border-purple-500/50 p-1 mb-4 relative shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                     ...
                     <div className="text-center mb-6">
                        ...
                        <div className="inline-flex items-center gap-2 mt-1">
                           <span className="text-[10px] text-purple-300 font-mono uppercase tracking-widest bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">Investor Tier 1</span>
                        </div>
                     </div>

                     <div className="w-full space-y-4">
                        <div>
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Nume Complet</label>
                           <div className="relative group">
                              <User className="absolute left-3 top-2.5 w-4 h-4 text-slate-600 group-focus-within:text-purple-400 transition-colors" />
                              <input type="text" defaultValue="Alex I." className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 pl-10 pr-3 text-sm text-slate-300 focus:outline-none focus:border-purple-500 transition-colors placeholder:text-slate-600" />
                           </div>
                        </div>
                        <div>
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Email</label>
                           <div className="relative group">
                              <Mail className="absolute left-3 top-2.5 w-4 h-4 text-slate-600 group-focus-within:text-purple-400 transition-colors" />
                              <input type="email" defaultValue="alex.investor@pi.ro" className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 pl-10 pr-3 text-sm text-slate-300 focus:outline-none focus:border-purple-500 transition-colors placeholder:text-slate-600" />
                           </div>
                        </div>
                        <div>
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1.5">ID Cont</label>
                           <div className="w-full bg-slate-900/50 border border-slate-800 rounded-lg py-2.5 px-4 text-sm text-slate-500 font-mono select-all cursor-pointer hover:text-slate-300 hover:border-slate-700 transition-colors">
                              USR-8821-X9
                           </div>
                        </div>
                     </div>
                  ...
                  <div className="bg-slate-950/50 p-3 border-t border-white/5 flex justify-between items-center text-[10px]">
               ...
               {/* Session Info - Comfortable Spacing */}
               <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-5 flex-1 flex flex-col">
                  ...
                  <div className="space-y-3">
                     <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyan-500/30 transition-colors">
                        <div className="flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse"></div>
                           <div>
                              <div className="text-xs font-bold text-white">Chrome - Windows</div>
                              <div className="text-[10px] text-slate-500 mt-0.5">București • 86.121.xx.xx</div>
                           </div>
                        </div>
                        <span className="text-[9px] text-emerald-500 font-mono border border-emerald-500/20 px-2 py-0.5 rounded bg-emerald-500/5">ACUM</span>
                     </div>
                     <div className="flex items-center justify-between p-3 bg-transparent rounded-xl border border-white/5 opacity-60 hover:opacity-100 transition-opacity">
                        <div className="flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full bg-slate-600"></div>
                           <div>
                              <div className="text-xs font-bold text-white">Safari - iPhone 14</div>
                              <div className="text-[10px] text-slate-500 mt-0.5">București • 2h în urmă</div>
                           </div>
                        </div>
                     </div>
                  </div>
                  <div className="mt-auto pt-4 text-center">
                     <button className="text-[10px] text-red-400 hover:text-red-300 transition-colors underline decoration-red-500/30 underline-offset-4">
                        Deconectează alte dispozitive
                     </button>
                  </div>
               </div>
            </div>
