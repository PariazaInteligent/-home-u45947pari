...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (dataLoading || !appData?.users) {
      ...
      // Debug logs pentru troubleshooting rapid
      console.log('[LoginPage] User returned from login:', user);
      console.log('[LoginPage] AppData:', appData);

      // Verificare: user trebuie să existe
      if (!user) {
        console.error('[LoginPage] Login returned null/undefined user');
      ...
      // Verificare: user.name trebuie să fie string (pentru a evita white/black screen)
      if (!user.name || typeof user.name !== 'string') {
        console.error('[LoginPage] User missing name property:', user);
        setError('Eroare: Datele utilizatorului sunt incomplete (lipsește numele).');
        addNotification('Eroare de autentificare: date utilizator incomplete.', NotificationType.ERROR);
        return;
      }

      // Verificare: user.role trebuie să existe
      if (!user.role) {
        console.error('[LoginPage] User missing role property:', user);
        setError('Eroare: Datele utilizatorului sunt incomplete (lipsește rolul).');
        addNotification('Eroare de autentificare: date utilizator incomplete.', NotificationType.ERROR);
        return;
      }

      // Dacă totul e ok, continuă logica normală
