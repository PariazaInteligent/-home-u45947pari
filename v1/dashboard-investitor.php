...
.mention-empty {
  ...
  padding: 0.5rem 0;
      ...
      <!-- Community Chat -->
      <div
  class="bg-glass-bg border border-glass-border rounded-xl flex flex-col relative overflow-hidden
  ...
  data-widget-id="chat">
      ...
      <div id="mentionsOverlay"
           class="hidden absolute inset-0 bg-slate-900/95 z-20 p-4 flex flex-col gap-3">
        ...
        <div id="mentionInbox"
             class="flex-1 overflow-y-auto space-y-2 text-xs nice-scroll">
        </div>
      </div>
 ...
 <!-- bara typing -->
      <div
        id="chatTypingBar"
        class="px-4 py-1 text-[11px] text-gray-500">
      </div>
  ...
  feed.querySelectorAll('.time[data-time][data-ts]').forEach(el => {
    ...
    el.textContent = formatRelativeTime(ts);
      ...
      function indexMessage(m) {
        ...
        // tăiem indexul dacă devine prea mare
  ...
  setView(true);
      ...
      function renderAttachments(list) {
        ...
        for (const att of list) {
          ...
          if (kind === 'image') {
            parts.push(`<a href="${url}" target="_blank" rel="noreferrer"><img src="${url}" alt="${name}" /></a>`);
          } else if (kind === 'video') {
            parts.push(`<video controls><source src="${url}" type="${mime}"></video>`);
          } else if (kind === 'audio') {
            parts.push(`<audio controls><source src="${url}" type="${mime}"></audio>`);
          } else {
            parts.push(`<a href="${url}" target="_blank" rel="noreferrer">📎 ${name}</a>`);
          }
        }
      ...
      function renderLinkPreview(preview) {
        ...
        return `
          <a href="${url}" target="_blank" rel="noreferrer" class="link-preview">
            <div class="link-body">
              ${imgHtml}
              <div class="link-text">
      ...
      async function sendReaction(messageId, emoji) {
        try {
          ...
          showChatToast('nu am putut aplica reacția.', 'error');
        }
      }
...
const userHtml = showUser
  ...
  <div class="bubble">
    ...
    <div class="react-bar">
      ...
      /* -------------------------- append / prepend mesaje -------------------------- */
        ...
        // confirmăm pending (client_id) – aici nu vrem sunete, e doar confirmare
        if (cid && PENDING.has(cid)) {
          return confirmPending(cid, m);
        }
        ...
        // considerăm „live” doar mesajele venite după bootstrap-ul inițial
        ...
        // mențiuni: refresh inbox + sunet de „mention”, dar doar dacă e live și nu ești tu
        if (hasMention && !mine && !VIEW_MENTIONS) {
          ...
          // restul mesajelor live de la alții: sunet de „incoming”
      ...
      async function startEditRow(row) {
        ...
        try {
          const r = await fetch('/api/chat/edit.php', {
            ...
            credentials: 'include',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrfToken || '' },
            body: JSON.stringify({ id: msgId, text: next.trim(), csrf_token: csrfToken || '' })
          });
          const j = await r.json().catch(() => null);
          if (!r.ok || !j || !j.ok) throw 0;
          await pullLatest();
        } catch {
          showChatToast('nu am putut edita mesajul.', 'error');
        }
      }
      ...
      async function deleteRow(row) {
        ...
        try {
          const r = await fetch('/api/chat/delete.php', {
            ...
            credentials: 'include',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrfToken || '' },
            body: JSON.stringify({ id: msgId, csrf_token: csrfToken || '' })
          });
          const j = await r.json().catch(() => null);
          if (!r.ok || !j || !j.ok) throw 0;
          await pullLatest();
        } catch {
          showChatToast('nu am putut șterge mesajul.', 'error');
        }
      }
  ...
  row.dataset.ts    = String(meta.ts || Math.floor(Date.now() / 1000));
  ...
  try {
    ...
    // acceptăm mai multe formate: [ ... ] sau { items: [...] } sau { ok, items }
    let items = [];
    if (Array.isArray(j)) {
      items = j;
